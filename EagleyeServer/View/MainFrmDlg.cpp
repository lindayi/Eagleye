...
//CGroup Group[7];
CMonitoredPC m_pPCArray[256];
//vector<CPolicy> Policy;
...
int CMainFrmDlg::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	//m_ServerConfig.SetFrequency(1000);
	/*
	CPolicy Def;
	m_ServerConfig.m_PolicyList.push_back(Def);
	m_ServerConfig.m_GroupList[0].m_Policy = &m_ServerConfig.m_PolicyList.at(0);
	m_ServerConfig.m_GroupList[0].m_Policy->m_BlackList.push_back("QQ.exe");
	m_ServerConfig.m_GroupList[0].m_Policy->m_BlackList.push_back("iexplore.exe");
	*/
	m_Conn.InitConnection(m_pPCArray, &m_ServerConfig, this);
